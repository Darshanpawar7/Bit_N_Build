"""Sarvam AI client for FlowState.
...
class SarvamClient:
    ...
    def __init__(self) -> None:
        ...
        self._mock_mode = not bool(
            self.api_key) or "your_" in self.api_key or requests is None
        if self._mock_mode:
            logger.warning(
    ...
    async def speech_to_text(self, audio_data: bytes, language: str = "english") -> dict[str, Any]:
        ...
        if not audio_data or len(audio_data) < 100:
            logger.warning(
                "STT called with empty/tiny audio (%d bytes)", len(audio_data))
            return {
                "transcript": "",
                "language": selected_language,
                "confidence": 0.0,
        ...
        try:
            ...
            logger.info("Calling Sarvam STT: %d bytes, lang=%s, content_type=%s", len(
                audio_data), lang_code, content_type)
            ...
            if response.status_code != 200:
                logger.error("Sarvam STT failed: status=%d, body=%s",
    ...
    async def text_to_speech(self, text: str, language: str = "english") -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("Calling Sarvam TTS: %d chars, lang=%s, speaker=%s", len(
                text), selected_language, payload["speaker"])
            ...
            if response.status_code != 200:
                logger.error("Sarvam TTS failed: status=%d, body=%s",
                             response.status_code, response.text[:500])
                return {
                    "audio_base64": "",
                    "language": selected_language,
            ...
            if audios and isinstance(audios, list) and len(audios) > 0:
                audio_b64 = audios[0]
                logger.info(
                    "Sarvam TTS success: audio base64 length=%d", len(audio_b64))
                return {
                ...
                logger.warning(
                    "Sarvam TTS returned empty audios array: %s", str(result)[:300])
                return {
                    "audio_base64": "",
                    "language": selected_language,
                    "text": text,
    ...
    async def translate(self, text: str, source_lang: str, target_lang: str) -> dict[str, Any]:
        ...
        try:
            ...
            logger.info("Calling Sarvam Translate: %s -> %s",
                        selected_source, selected_target)
            ...
            if response.status_code != 200:
                logger.error("Sarvam Translate failed: status=%d, body=%s",
            ...
            logger.info("Sarvam Translate success: '%s' -> '%s'",
                        text[:50], str(translated)[:50])
